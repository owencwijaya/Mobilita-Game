/**
 * @deprecated 
 * @brief This function is no longer used, use
 *        parseConfig instead.
 * @see "./modules/io/config_parser.c"
 */
...
{ //ini nanti sesuaiin sama struct Raden
    ...
    int cols;
    Matrix map;
    rows = currentToken.val;
    advToken();
    cols = currentToken.val;
    advToken();
    ...
    int HQX;
    int HQY;
    HQX = currentToken.val;
    advToken();
    HQY = currentToken.val;
    advToken();
    ELMT(map, HQX - 1, HQY - 1) = '8';

    /* akuisisi jumlah query */
    int query = currentToken.val;
    advToken();
    int i = 0;
    int j = 0;
    int tempX;
    ...
    for (i = 0; i < query; i++)
    {
        CharLocation = currentToken.tkn;
        advToken();
        tempX = currentToken.val;
        advToken();
        tempY = currentToken.val;
        advToken();
