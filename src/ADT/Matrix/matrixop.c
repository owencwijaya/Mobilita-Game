...
/* 
    ...
    Topik: ADT Matriks
    Deskripsi: Implementasi ADT Matriks dan 
    penambahan fungsi primitif matriks
*/
...
float rowMean(Matrix m, Index i){
    /* Mencari rata-rata nilai baris i */
    /* Kamus */
    int j = 0;
    float count = 0;
    float sum = 0;
...
float colMean(Matrix m, Index j){
    /* Mencari rata-rata nilai kolom j */
    /* Kamus */
    int i = 0;
    float count = 0;
    float sum = 0;
...
void rowExtremes(Matrix m, Index i, ElType *max, ElType *min){
    /* Mencari nilai maksimum dan minimum di baris i */
    /* Kamus */
    ElType localMax = ELMT(m, i, 0);
    ElType localMin = ELMT(m, i, 0);
    int j = 0;
    /* Algoritma */
    for(j = 0; j < COLS(m); j++){
        if(ELMT(m, i, j) <= localMin){
            localMin = ELMT(m, i, j);
        }
        if(ELMT(m, i, j) >= localMax){
            localMax = ELMT(m, i, j);
        }
    }
...
void colExtremes(Matrix m, Index j, ElType *max, ElType *min){
    /* Mencari nilai maksimum dan minimum di kolom j */
    /* Kamus */
    ElType localMax = ELMT(m, 0, j);
    ElType localMin = ELMT(m, 0, j);
...
int countValOnRow(Matrix m, Index i, ElType val){
    /* Mencari banyaknya kemunculan nilai val di baris i */
    /* Kamus */
    int j = 0;
    int count = 0;
...
int countValOnCol(Matrix m, Index j, ElType val){
    /* Mencari banyaknya kemunculan nilai val di kolom j */
    /* Kamus */
    int i = 0;
    int count = 0;
